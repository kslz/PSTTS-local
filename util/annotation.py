...
def annotation_dataset(data_dir, label_path, temp_dir="/home/aistudio/work/temp", use_ffmpeg=True):
    # 先清空 temp 目录
...
def annotation_dataset_spks(data_dir, label_path, temp_dir="/home/aistudio/work/temp", use_ffmpeg=True):
    # 先清空 temp 目录
    if os.path.exists(temp_dir):
        shutil.rmtree(temp_dir)
    os.makedirs(temp_dir, exist_ok=True)
    ...
    spks = [dirname for dirname in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir,dirname)) and dirname != ".ipynb_checkpoints"]
    print(spks)
    for spk in spks:
        os.makedirs(os.path.join(temp_dir,spk), exist_ok=True)
        ann_result = []
        wavs = [filename for filename in os.listdir(os.path.join(data_dir,spk)) if filename[-4:] in ['.wav', '.mp3', '.ogg']]
        print(wavs)
        if len(wavs) < 5:
            print(f"{spk}数据小于5句，不建议微调，请添加数据")
            return
        for idx, filename in tqdm.tqdm(enumerate(wavs)):
            # 检查文件名中是否有非法字符，存在非法字符则重新命名
            input_path = os.path.join(data_dir, spk, filename)
            if " " in filename:
                new_filename = str(idx) + "_" + random_string_generator(4) + ".wav"
                filename = new_filename
                new_file_path = os.path.join(data_dir, new_filename)
            ...
            if use_ffmpeg:
                # 使用 ffmpeg 统一音频格式
                output_path = os.path.join(temp_dir, spk, filename)
                output_path = trans_wav_format(input_path, output_path)
                filepath = output_path
            else:
                filepath = input_path
        ...
        status_change(ann_json, False)
        ann_result = sorted(ann_result, key=lambda x:x['filename'])
        ann_result_dict[spk]=ann_result

    print(ann_result_dict)
        
    if ann_result_dict:
        if label_path:
            with open(label_path, "w", encoding="utf8") as f:
                json.dump(ann_result_dict, f, indent=2, ensure_ascii=False)
    else:
        return None
    return ann_result
